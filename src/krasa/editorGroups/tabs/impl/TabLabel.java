...
import krasa.editorGroups.tabs.TabsUtil;
import krasa.editorGroups.tabs.UiDecorator;
...
public class TabLabel extends JPanel implements Accessible {
	...
	private SimpleColoredComponent createLabel(final JBTabsImpl tabs) {
		...
		label.setIconTextGap(tabs.isEditorTabs() ? (!getHideTabsIfNeed() ? 4 : 2) : new JLabel().getIconTextGap());
	...
	private boolean getHideTabsIfNeed() {
		return true;
	}
	...
	public Insets getInsets() {
		...
		if (myTabs.isEditorTabs() && getShowCloseButton()) {
			if (!getCloseTabButtonOnTheRight()) {
				insets.left = 3;
			} else {
				insets.right = 3;
			}
	...
	private boolean getCloseTabButtonOnTheRight() {
		return false;
	}
	...
	public void setTabActions(ActionGroup group) {
		...
		myActionPanel = new ActionPanel(myTabs, myInfo, new Pass<MouseEvent>() {
			...
			public void pass(final MouseEvent event) {
				final MouseEvent me = SwingUtilities.convertMouseEvent(event.getComponent(), event, krasa.editorGroups.tabs.impl.TabLabel.this);
		...
		add(myActionPanel, getCloseTabButtonOnTheRight() ? BorderLayout.EAST : BorderLayout.WEST);
