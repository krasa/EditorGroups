...
//	private IdeGlassPane myGlassPane;
  ...
  public JBTabsImpl(@Nullable Project project, @NotNull ActionManager actionManager, IdeFocusManager focusManager, @NotNull Disposable parent) {
    ...
    addMouseWheelListener(event -> {
      ...
      if (SystemInfo.isMac && event.getModifiersEx() == InputEvent.SHIFT_DOWN_MASK) return;
    ...
    ClientProperty.put(
      this, UIUtil.NOT_IN_HIERARCHY_COMPONENTS, (Iterable<JComponent>) () -> JBIterable.from(getVisibleInfos()).filter(Conditions.not(Conditions.is(mySelectedInfo))).transform(
        TabInfo::getComponent).iterator());
  }
  ...
  private void processFocusChange() {
    ...
    setFocused(owner == this || SwingUtilities.isDescendingFrom(owner, this));
  }
  ...
  protected void revalidateAndRepaint(final boolean layoutNow) {
    if (myVisibleInfos.isEmpty()) {
      ...
      if (getParent() != null) {
        ...
        Objects.requireNonNull(nonOpaque).repaint(toRepaint.x, toRepaint.y, toRepaint.width, toRepaint.height);
      }
    } else {
      setOpaque(true);
    }
...
//			myTabActionsAutoHideListener.processMouseOver();
  ...
  public Dimension getMinimumSize() {
    return computeSize(JComponent::getMinimumSize, 1);
  }

  @Override
  public Dimension getPreferredSize() {
    return computeSize(JComponent::getPreferredSize, 3);
  }
  ...
  @Override
  public TabInfo findInfo(final MouseEvent event) {
