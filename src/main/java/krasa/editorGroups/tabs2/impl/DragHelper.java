...
class DragHelper extends MouseDragHelper {
  ...
  private Reference<krasa.editorGroups.tabs2.impl.TabLabel> myPressedTabLabel;
  ...
  protected boolean isDragOut(@NotNull MouseEvent event, @NotNull Point dragToScreenPoint, @NotNull Point startScreenPoint) {
    ...
    krasa.editorGroups.tabs2.impl.TabLabel label = myTabs.myInfo2Label.get(myDragSource);
  ...
  protected void processMousePressed(@NotNull MouseEvent event) {
    ...
    krasa.editorGroups.tabs2.impl.TabLabel label = findLabel(new RelativePoint(event).getPoint(myTabs));
  ...
  protected void processDrag(@NotNull MouseEvent event, @NotNull Point targetScreenPoint, @NotNull Point startPointScreen) {
    ...
    if (isDragJustStarted()) {
      krasa.editorGroups.tabs2.impl.TabLabel pressedTabLabel = SoftReference.dereference(myPressedTabLabel);
    ...
    final krasa.editorGroups.tabs2.impl.TabLabel top = findLabel(new Point(myDragRec.x + myDragRec.width / 2, myDragRec.y + deadZoneY));
    final krasa.editorGroups.tabs2.impl.TabLabel bottom = findLabel(new Point(myDragRec.x + myDragRec.width / 2, myDragRec.y + myDragRec.height - deadZoneY));
    final krasa.editorGroups.tabs2.impl.TabLabel left = findLabel(new Point(myDragRec.x + deadZoneX, myDragRec.y + myDragRec.height / 2));
    final krasa.editorGroups.tabs2.impl.TabLabel right = findLabel(new Point(myDragRec.x + myDragRec.width - deadZoneX, myDragRec.y + myDragRec.height / 2));


    krasa.editorGroups.tabs2.impl.TabLabel targetLabel;
  ...
  private krasa.editorGroups.tabs2.impl.TabLabel findMostOverlapping(Axis measurer, krasa.editorGroups.tabs2.impl.TabLabel... labels) {
    double freeSpace;
    ...
    int max = -1;
    krasa.editorGroups.tabs2.impl.TabLabel maxLabel = null;
  ...
  @Nullable
  private krasa.editorGroups.tabs2.impl.TabLabel findLabel(Point dragPoint) {
    final Component at = myTabs.findComponentAt(dragPoint);
    if (at instanceof InplaceButton) return null;
    final krasa.editorGroups.tabs2.impl.TabLabel label = findLabel(at);
  ...
  @Nullable
  private krasa.editorGroups.tabs2.impl.TabLabel findLabel(Component c) {
    Component eachParent = c;
    while (eachParent != null && eachParent != myTabs) {
      if (eachParent instanceof krasa.editorGroups.tabs2.impl.TabLabel) return (TabLabel) eachParent;
