...
public class SingleRowLayout extends TabLayout {
  ...
  public final krasa.editorGroups.tabs2.impl.singleRow.MoreTabsIcon myMoreIcon = new MoreTabsIcon() {
    @Override
    @Nullable
  ...
  protected void recomputeToLayout(final SingleRowPassInfo data) {
    ...
    while (true) {
      ...
      if (data.toLayout.isEmpty()) break;
  ...
  public int getDropIndexFor(Point point) {
    ...
    if (c instanceof JBTabsImpl) {
      for (int i = 0; i < myLastSingRowLayout.myVisibleInfos.size() - 1; i++) {
        ...
        final boolean between = isBetween(point, first, second);
    ...
    if (c instanceof TabLabel) {
    ...
    } else if (c instanceof GhostComponent ghost) {
      ...
      if (info != null) {
        ...
        if (myLastSingRowLayout.myVisibleInfos.isEmpty()) {
          result = 0;
        } else {
  ...
  private boolean isBetween(Point point, TabLabel first, TabLabel second) {
