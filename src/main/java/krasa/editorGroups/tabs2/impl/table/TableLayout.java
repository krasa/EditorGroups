...
import krasa.editorGroups.tabs2.impl.LayoutPassInfo;
...
public class TableLayout extends TabLayout {
	...
	private TablePassInfo computeLayoutTable(List<TabInfo> visibleInfos) {
		...
		krasa.editorGroups.tabs2.impl.table.TableRow eachTableRow = new krasa.editorGroups.tabs2.impl.table.TableRow(data);
		...
		for (TabInfo eachInfo : data.myVisibleInfos) {
			...
			if (eachX + size.width <= data.rowToFitMaxX) {
				...
				eachTableRow = new krasa.editorGroups.tabs2.impl.table.TableRow(data);
				data.table.add(eachTableRow);
				eachRow++;
		...
		List<krasa.editorGroups.tabs2.impl.table.TableRow> toMove = new ArrayList<>();
	...
	public boolean isLastRow(TabInfo info) {
		...
		List<krasa.editorGroups.tabs2.impl.table.TableRow> rows = myLastTableLayout.table;
		if (rows.size() > 0) {
	...
	public LayoutPassInfo layoutTable(List<TabInfo> visibleInfos) {
		...
		for (krasa.editorGroups.tabs2.impl.table.TableRow eachRow : data.table) {
			eachX = insets.left;
