...
import krasa.editorGroups.tabs2.JBTabsEx;
import krasa.editorGroups.tabs2.TabInfo;
import krasa.editorGroups.tabs2.UiDecorator;
...
public class TabLabel extends JPanel implements Accessible, Disposable {
  ...
  public void apply(UiDecorator.UiDecoration decoration) {
    ...
    if (insets != null) {
      ...
      if (current != null) {
        setBorder(
          JBUI.Borders.empty(getValue(current.top, insets.top), getValue(current.left, insets.left), getValue(current.bottom, insets.bottom), getValue(current.right, insets.right)));
      }
    }
  }
